...
Comments are not directly supported. However, you could always add a
comment definition, e.g.,

    = Comment {<comment> str}
    # or
    = Comment {<#> str}

Or, simply add an extra `<comment> str` or ``<#> str`` field to one or more
