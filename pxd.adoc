...
`recordlist`, `recorddict`, or a single `record`.
    ...
    HEADER     ::= "'pxd'" OWS "[" OWS /\d\d?\.\d\d?/ (RWS TEXT)? OWS "]"
    ...
    DATA       ::= "'data'" OWS
                   (LIST | DICT | TYPEDLIST | RECORDLIST | RECORDDICT |
                    RECORD)
    LIST       ::= "[" OWS VALUE? (RWS VALUE)* OWS "]"
    DICT       ::= "{" OWS (KEY RWS VALUE)? (RWS KEY RWS VALUE)* OWS "}"
    TYPEDLIST ::= ("intlist" | "datelist" | "datetimelist" | "byteslist" |
                   "reallist" | "textlist") LIST
